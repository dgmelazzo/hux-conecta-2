...
function conectaApi(string $action, array $data): array {
    $url = 'https://conecta.acicdf.org.br/auth.php?action=' . $action;
    $ch  = curl_init($url);
...
// ============================================================
// AUTH — login de usuários internos (gestor) e
...
if ($method === 'POST' && $uri === '/auth/sso-conecta') {
    ...
    $ch = curl_init('https://conecta.acicdf.org.br/auth.php?action=validate');
    curl_setopt_array($ch, [
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_TIMEOUT        => 15,
        CURLOPT_POST           => true,
...
// ── POST /auth/criar-usuario — cria gestor (só superadmin/gestor) ───
if ($method === 'POST' && $uri === '/auth/criar-usuario') {
    ...
    if (!in_array($b['role'], ['gestor'])) {
        json_out(['error' => 'Role inválida. Use: gestor'], 422);
    }
...
if ($method === 'POST' && preg_match('#^/associados/(\d+)/redefinir-senha$#', $uri, $m)) {
    ...
    if ($docClean) {
        $ch = curl_init('https://conecta.acicdf.org.br/auth.php?action=reset_password');
        curl_setopt_array($ch, [
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_TIMEOUT        => 15,
            CURLOPT_POST           => true,

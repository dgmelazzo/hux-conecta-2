...
switch ($action) {
    ...
    case 'parceiros':
        $rows = getDB()->query("
            SELECT id, nome, nome_fantasia, cnpj, email, telefone, site, logo_url, categoria, descricao, ativo
    ...
    case 'parceiro_criar':
        ...
        getDB()->prepare("
            INSERT INTO conecta_parceiros (nome, nome_fantasia, cnpj, email, telefone, site, logo_url, categoria, descricao)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ")->execute([
            trim($in['nome']),
    ...
    case 'parceiro_editar':
        ...
        if (empty($in['id'])) err(400, 'ID obrigatório.');
        getDB()->prepare("
            UPDATE conecta_parceiros SET nome=?, nome_fantasia=?, cnpj=?, email=?, telefone=?, site=?, logo_url=?, categoria=?, descricao=?
            WHERE id=?
        ")->execute([
            trim($in['nome'] ?? ''),
            trim($in['nome_fantasia'] ?? ''),

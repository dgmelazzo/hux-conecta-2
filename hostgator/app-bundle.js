...
const AUTH_URL = 'https://acicdf.org.br/conecta/auth.php'; // ← ajuste se necessário
...
async function apiGetAssociado() {
  ...
  if (session?.is_admin === true) {
    return {
      ...
      razaoSocial:    data.nome || session?.nome || 'Administrador',
      nomeFantasia:   data.nome || session?.nome || 'Administrador',
      cnpj:           data.cpf_cnpj || '—',
      email:          '—',
      telefone:       '—',
      site:           '—',
...
const PRODUTOS_URL = 'https://acicdf.org.br/conecta/produtos.php';
    ...
    <div class="produto-card" onclick="abrirProduto('${p.slug}')">
      ...
      <div class="produto-body">
        ...
        <div class="produto-footer">
          <span class="produto-empresa">${p.associado_nome || ''}</span>
  ...
  return `
    ...
    <div class="modal-produto-header" style="margin-top:${temGaleria||p.imagem?'16px':'0'}">
      <div class="modal-produto-info">
        ...
        ${p.associado_nome ? `<p class="modal-empresa"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg> ${p.associado_nome}</p>` : ''}
      </div>
    </div>
...
function _restaurarSecao() {
  ...
  const adminSecs = ['admin-produtos','admin-usuarios','admin-metricas','admin-admins','admin-comunicados'];
...
function showPortal() {
  ...
  ['nav-admin','nav-usuarios','nav-metricas','nav-admins'].forEach(id => {
...
function showSection(id) {
  ...
  if (adminSections.includes(id)) {
    ...
    if (id === 'admin-usuarios') carregarUsuarios();
...
function statusLabel(s) {
  ...
  return map[s] || (s ? s.charAt(0).toUpperCase() + s.slice(1).toLowerCase() : '—');
}
...
function mostrarNavAdmin(isSuperadmin) {
  ...
  // Menu Admins apenas para superadmin
  const navAdmins = document.getElementById('nav-admins');
  if (navAdmins) {
    if (isSuperadmin) navAdmins.classList.remove('hidden');
    else navAdmins.classList.add('hidden');
  }
}
...
function checkAdminAccess() {
  ...
  .then(r => {
    if (r.success === true && r.data?.is_admin === true) {
      ...
      sessionStorage.setItem('acic_session', JSON.stringify(s));
...
// ── UPLOAD DE IMAGEM ─────────────────────────────────────────
const UPLOAD_URL = 'https://acicdf.org.br/conecta/upload.php';
...
// ════════════════════════════════════════════════════════════
const ADMIN_URL = 'https://acicdf.org.br/conecta/admin.php';
...
function revelarMenusAdmin() {
  ['nav-usuarios','nav-metricas'].forEach(id => {
    document.getElementById(id)?.classList.remove('hidden');
  });
}

// ── showSection: registra admin sections ─────────────────
const _adminSections = ['admin-usuarios','admin-metricas'];
...
// ============================================================
const NOTIF_URL = 'https://acicdf.org.br/conecta/notificacoes.php';

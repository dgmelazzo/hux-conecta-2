...
<title>Minhas Cobranças — Conecta ACIC</title>
...
<header class="topbar">
  ...
  <div class="topbar-title">Minhas Cobranças</div>
...
if (token) {
  ...
  if (cartLink) cartLink.href = '/conecta/portal-carteirinha.php?token=' + encodeURIComponent(token);
}
...
(async function() {
  if (!token) {
    document.getElementById('page-content').innerHTML =
      '<div class="empty-state"><h3>Sessão expirada</h3><p>Faça login novamente para ver suas taxas.</p><a href="/conecta/" class="cob-btn-pay" style="text-decoration:none">Ir para o login</a></div>';
    return;
  }
